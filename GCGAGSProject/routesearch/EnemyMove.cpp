...
void EnemyMove::Init(mapChipDate date, Vector2 mapsize)
{
	goalPos_ = { 0,0 };
	startPos_ = { 0,0 };
	mapSize_ = { 0,0 };
...
int EnemyMove::StartCheck(int x, int y)
{
	...
	goalRoute_.clear();
	goalRoute_ = goalRouteList_[0].first;
	return true;
}

bool EnemyMove::CheckMapPoint(int x, int y, int num)
{
	if (x >= mapSize_.x)
	{
		return false;
...
std::list<Vector2> EnemyMove::CheckMap(int x, int y, int point)
{
	//もし現在地がゴールならば
	if (goalPos_.x == x && goalPos_.y == y)
	{
	...
	if (mapdate_[y][x] == BlockDate::Gate)
	{
		map_[y][x].StepCost = 50;
	}
	if (mapdate_[y][x] == BlockDate::Spike)
	{
		map_[y][x].StepCost = 3;
	}
