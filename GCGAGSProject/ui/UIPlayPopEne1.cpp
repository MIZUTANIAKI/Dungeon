...
UIPlayPopEne1::UIPlayPopEne1()
{
	SetDate();
}

UIPlayPopEne1::UIPlayPopEne1(Vector2 pos, Vector2 size) :UIBase(pos, size)
{
	subUIDispF_ = false;
	subUi_ = -1;
	mousecount_ = 0;
...
void UIPlayPopEne1::Update(void)
{
		...
		nowMPos.y <= pos_.y + size_.y)
	{
		...
		if (!subUIDispF_)
		{
			if (mousecount_ > 60)
			{
				...
				lpUIMng.SetUIDate(subUi_, "ñ°ï˚è¢ä´1");
				subUIDispF_ = true;
			}
		}
...
void UIPlayPopEne1::SetDate(void)
{
}

void UIPlayPopEne1::Draw(void)
{
	Reset();
	...
	DrawGraph(size_.x / 2 - 32 / 2, size_.y / 2 - 32, lpImglMng.GetGraphHandle("testC2.png"), true);
